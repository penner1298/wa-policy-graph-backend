...
def process_city(jurisdiction, start_date, end_date):
    print(f"\nFetching Meetings for {jurisdiction} from {start_date} to {end_date}...")
    url = f"https://webapi.legistar.com/v1/{jurisdiction}/events?$filter=EventDate ge datetime'{start_date}T00:00:00' and EventDate le datetime'{end_date}T23:59:59'&$orderby=EventDate desc"
    
    resp = requests.get(url, headers={"Accept": "application/json"})
    if resp.status_code != 200: 
        print(f"Failed to fetch {jurisdiction}. Status: {resp.status_code}")
        return
    events = resp.json()
    ...
    for i, event in enumerate(completed):
        ...
        print(f"[{i+1}/{len(completed)}] {jurisdiction.capitalize()} - {committee} on {date[:10]}")
        ...
        try:
            ...
            conn.commit()
            print(f"  -> Action: {res['key_action'][:80]}...")
...
def main():
    # Granicus targets (Major WA jurisdictions)
    jurisdictions = ["seattle", "tacoma", "kingcounty", "snohomishcounty", "spokane", "bellevue", "everett"]
    for j in jurisdictions:
        process_city(j, "2024-01-01", "2026-12-31")
